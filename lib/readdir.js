...
var invoke         = require('es5-ext/function/invoke')
  ...
  , assignMultiple = require('es5-ext/object/assign-multiple')
...
Readdir.prototype = {
	...
	filterByType: function (paths, getDirs) {
		...
		resolve = function (e) {
			if (defFiles) defFiles.resolve(e || files);
			if (defDirs) defDirs.resolve(e || dirs);
			failed = Boolean(e);
			resolved = true;
		};
	...
	filterIgnored: function (paths) {
		...
		if (this.watch) {
			...
			promise.close = function () {
				if (promises) {
					if (!def.resolved) def.resolve(new Error("Operation aborted"));
					forEach(promises, invoke('close'));
					promises = null;
					paths.close();
				}
	...
	readdir: function (path) {
		...
		if (this.watch) {
			...
			watcher.on('end', function (err) {
				if (!def.resolved) def.resolve(err);
			...
			promise.close = function () {
				watcher.close();
				if (!def.resolved) def.resolve(new Error("Readdir action cancelled"));
			};
		}
		original(path, function (err, data) {
			if (err) {
...
readdir = function (path, options) {
	...
	if (options.ignoreRules) {
		assignMultiple(readdir, getIsIgnored(isArray(options.ignoreRules) ?
